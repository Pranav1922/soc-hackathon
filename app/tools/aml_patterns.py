...
class AMLPatternDetector(Tool):
    ...
    def _detect_structuring(self, df: pd.DataFrame) -> list[dict[str, Any]]:
        ...
        for customer_id, grp in candidates.groupby("customer_id", sort=False):
            ...
            for right in range(len(grp)):
                t_right = grp.loc[right, "timestamp"]
                while grp.loc[left, "timestamp"] < t_right - window:
                    left += 1
                count = right - left + 1
    ...
    def _detect_smurfing(self, df: pd.DataFrame) -> list[dict[str, Any]]:
        ...
        for customer_id, grp in candidates.groupby("customer_id", sort=False):
            ...
            for right in range(len(grp)):
                source_counts[grp.loc[right, "counterparty_id"]] += 1
                t_right = grp.loc[right, "timestamp"]
                while grp.loc[left, "timestamp"] < t_right - window:
                    cp = grp.loc[left, "counterparty_id"]
                    source_counts[cp] -= 1
                ...
                if len(source_counts) >= cfg.min_distinct_sources:
                    ...
                    hits.append(
                        self._make_hit(
                            ...
                            evidence={
                                "txn_ids": window_rows["txn_id"].tolist(),
                                "distinct_sources": int(len(source_counts)),
    ...
    def _detect_layering(self, df: pd.DataFrame) -> list[dict[str, Any]]:
        ...
        for row in edges_df.itertuples(index=False):
            edges_by_source[row.customer_id].append(
                {
        ...
        def dfs(
            ...
            if len(path_edges) >= cfg.min_hops:
                if path_edges[-1]["timestamp"] - path_edges[0]["timestamp"] <= window:
